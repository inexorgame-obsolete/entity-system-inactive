...
using LowerThanInt = entity_system::type_system::LowerThanInt;
...
LowerThanIntProcessor::LowerThanIntProcessor(EntityTypeManagerPtr entity_type_manager, EntityInstanceManagerPtr entity_instance_manager, LogManagerPtr log_manager)
...
LowerThanIntProcessor::~LowerThanIntProcessor() = default;
...
void LowerThanIntProcessor::init_processor()
{
    EntityTypePtrOpt o_ent_type = entity_type_manager->get_entity_type(LowerThanInt::TYPE_NAME);
    if (o_ent_type.has_value()) {
        ...
        spdlog::get(LOGGER_NAME)->error("Failed to initialize processor {}: Entity type does not exist", LowerThanInt::TYPE_NAME);
    }
}

std::string LowerThanIntProcessor::get_component_name()
{
    return "LowerThanIntProcessor";
}
...
void LowerThanIntProcessor::make_signals(const EntityInstancePtr &entity_instance)
{
    ...
    auto o_lower_than_int_input_1 = entity_instance->get_attribute_instance(LowerThanInt::INPUT_1);
    auto o_lower_than_int_input_2 = entity_instance->get_attribute_instance(LowerThanInt::INPUT_2);
    auto o_lower_than_int_result = entity_instance->get_attribute_instance(LowerThanInt::RESULT);

    if (o_lower_than_int_input_1.has_value() && o_lower_than_int_input_2.has_value() && o_lower_than_int_result.has_value())
    {
        signals[entity_instance->get_GUID()] = MakeSignal(With(o_lower_than_int_input_1.value()->value, o_lower_than_int_input_2.value()->value), [](DataValue lower_than_int_input_1, DataValue lower_than_int_input_2) {
    ...
    } else
    {
        spdlog::get(LOGGER_NAME)
            ->error("Failed to initialize processor signals for entity instance {} of type {}: Missing one of these attributes: {} {} {}", entity_instance->get_GUID().str(), entity_instance->get_entity_type()->get_type_name(),
                    LowerThanInt::INPUT_1, LowerThanInt::INPUT_2, LowerThanInt::RESULT);
    }
}
