...
namespace visual_scripting {
	...
	GreaterThanIntProcessor::GreaterThanIntProcessor(
	...
	void GreaterThanIntProcessor::make_signals(const EntityInstancePtr& entity_instance)
	{
        ...
        if(o_greater_than_int_input_1.has_value() && o_greater_than_int_input_2.has_value() && o_greater_than_int_result.has_value())
		{
			signals[entity_instance->get_GUID()] = MakeSignal(
				With(
					o_greater_than_int_input_1.value()->value,
					o_greater_than_int_input_2.value()->value
				),
				[] (DataValue greater_than_int_input_1, DataValue greater_than_int_input_2)
				{
        ...
        {
			spdlog::get(LOGGER_NAME)->error("Failed to initialize processor signals for entity instance {} of type {}: Missing one of these attributes: {} {} {}", entity_instance->get_GUID().str(), entity_instance->get_entity_type()->get_type_name(), GreaterThanIntEntityTypeProvider::GREATER_THAN_INT_INPUT_1, GreaterThanIntEntityTypeProvider::GREATER_THAN_INT_INPUT_2, GreaterThanIntEntityTypeProvider::GREATER_THAN_INT_RESULT);
		}
	}
