...
NorProcessor::NorProcessor(const NorEntityTypeProviderPtr &entity_type_provider, EntityInstanceManagerPtr entity_instance_manager, LogManagerPtr log_manager)
    ...
    } else
    {
        spdlog::get(LOGGER_NAME)
            ->error("Failed to initialize processor signals for entity instance {} of type {}: Missing one of these attributes: {} {} {}", entity_instance->get_GUID().str(), entity_instance->get_entity_type()->get_type_name(),
                    NorEntityTypeProvider::NOR_INPUT_1, NorEntityTypeProvider::NOR_INPUT_2, NorEntityTypeProvider::NOR_RESULT);
    }
}
