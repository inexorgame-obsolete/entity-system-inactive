...
namespace visual_scripting {
	...
	void OrProcessor::make_signals(const EntityInstancePtr& entity_instance)
	{
        ...
        auto o_or_input_1 = entity_instance->get_attribute_instance(OrEntityTypeProvider::OR_INPUT_1);
		auto o_or_input_2 = entity_instance->get_attribute_instance(OrEntityTypeProvider::OR_INPUT_2);
		auto o_or_result = entity_instance->get_attribute_instance(OrEntityTypeProvider::OR_RESULT);
		
        if(o_or_input_1.has_value() && o_or_input_2.has_value() && o_or_result.has_value())
		{
			signals[entity_instance->get_GUID()] = MakeSignal(
				With(
					o_or_input_1.value()->value,
					o_or_input_2.value()->value
				),
				[] (DataValue or_input_1, DataValue or_input_2)
				{
					return DataValue(std::get<DataType::BOOL>(or_input_1) || std::get<DataType::BOOL>(or_input_2));
				}
			);
        ...
        {
			spdlog::get(LOGGER_NAME)->error("Failed to initialize processor signals for entity instance {} of type {}: Missing one of these attributes: {} {} {}", entity_instance->get_GUID().str(), entity_instance->get_entity_type()->get_type_name(), OrEntityTypeProvider::OR_INPUT_1, OrEntityTypeProvider::OR_INPUT_2, OrEntityTypeProvider::OR_RESULT);
		}
	}
