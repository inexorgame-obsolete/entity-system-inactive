...
namespace visual_scripting {
	...
	void TanProcessor::make_signals(const EntityInstancePtr& entity_instance)
	{
		...
		std::optional<std::shared_ptr<inexor::entity_system::EntityAttributeInstance>> o_attr_tan_value = entity_instance->get_attribute_instance(TanEntityTypeProvider::TAN_VALUE);
		if (o_attr_tan_value.has_value())
		{
			xg::Guid guid = entity_instance->get_GUID();
			std::shared_ptr<inexor::entity_system::EntityAttributeInstance> attr_tan_value = o_attr_tan_value.value();
		    std::thread start_thread([this, guid, attr_tan_value] () {
			    ...
			    signals[guid] = MakeSignal(
					...
					[resolution] (int time_iterator)
					{
						...
						float y_value = tan(x_value);
						return entity_system::DataValue(y_value);
					}
				);

			    // Attach the signal to the signal_wrapper
				// o_attr_tan_value.value()->value = signals[guid];
			    attr_tan_value->signal_wrapper <<= signals[guid];
			...
			spdlog::get(LOGGER_NAME)->error("Failed to initialize processor signals for entity instance {} of type {}: Missing attribute {}", entity_instance->get_GUID().str(), entity_instance->get_entity_type()->get_type_name(), TanEntityTypeProvider::TAN_VALUE);
		}
	}
