...
using FloatConstant = entity_system::type_system::FloatConstant;
...
void SinTest::run_test()
{
    ...
    EntityAttributeInstancePtrOpt o_counter_attr_count = counter->get_attribute_instance(CounterFloat::COUNT);
    EntityAttributeInstancePtrOpt o_sin_attr_input = sin->get_attribute_instance(Sin::INPUT);
    EntityAttributeInstancePtrOpt o_sin_attr_value = sin->get_attribute_instance(Sin::VALUE);
    EntityAttributeInstancePtrOpt o_target_attr_value = target->get_attribute_instance(FloatConstant::VALUE);
...
void SinTest::create_instances()
{
    ...
    o_sin = entity_instance_builder_factory->get_builder(Sin::TYPE_NAME)->attribute(Sin::INPUT, 0.0f)->attribute(Sin::VALUE, 0.0f)->build();
...
{
    log_create_connector(COUNTER_NAME, CounterFloat::COUNT, SIN_NAME, Sin::INPUT);
    ...
    log_create_connector(SIN_NAME, Sin::VALUE, TARGET_NAME, FloatConstant::VALUE);
...
void SinTest::log_attrs(const std::string& message)
{
    ...
    log_attr(COUNTER_NAME, CounterFloat::COUNT, counter_attr_count);
    log_attr(SIN_NAME, Sin::INPUT, sin_attr_input);
    log_attr(SIN_NAME, Sin::VALUE, sin_attr_value);
    log_attr(TARGET_NAME, FloatConstant::VALUE, target_attr_value);
}
