...
GlobalKeyProcessor::GlobalKeyProcessor(const GlobalKeyEntityTypeProviderPtr &entity_type_provider, EntityInstanceManagerPtr entity_instance_manager, KeyboardInputManagerPtr keyboard_input_manager, LogManagerPtr log_manager)
    : Processor(entity_type_provider->get_type()), entity_type_provider(entity_type_provider), entity_instance_manager(std::move(entity_instance_manager)), keyboard_input_manager(std::move(keyboard_input_manager)),
    ...
    } else
    {
        spdlog::get(LOGGER_NAME)
            ->error("Failed to initialize processor signals for entity instance {} of type {}: Missing one of these attributes {} {} {}", entity_instance->get_GUID().str(), entity_instance->get_entity_type()->get_type_name(),
                    GlobalKeyEntityTypeProvider::GLOBAL_KEY_KEYCODE, GlobalKeyEntityTypeProvider::GLOBAL_KEY_ACTION, GlobalKeyEntityTypeProvider::GLOBAL_KEY_MODS);
    }
}
