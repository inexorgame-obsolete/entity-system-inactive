...
EntityTypeManager::EntityTypeManager() = default;
...
void EntityTypeManager::register_on_created(const std::shared_ptr<EntityTypeCreatedListener> &listener)
{
...
void EntityTypeManager::register_on_deleted(const std::shared_ptr<EntityTypeDeletedListener> &listener)
{
...
{
    signal_entity_type_created(std::move(new_entity_type));
}
