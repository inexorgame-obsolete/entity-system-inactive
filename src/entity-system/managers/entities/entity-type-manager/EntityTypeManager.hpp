...
class EntityTypeManager : public TypeManagerTemplate<EntityType>
{
    ...
    void register_on_created(const std::shared_ptr<EntityTypeCreatedListener>& listener);

    /// @brief Registers a new listener
    /// @param listener The listener.
    void register_on_deleted(const std::shared_ptr<EntityTypeDeletedListener>& listener);
