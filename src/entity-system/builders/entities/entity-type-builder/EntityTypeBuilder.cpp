	...
	O_ENT_TYPE EntityTypeBuilder::build()
	{
		...
		if (o_entity_type.has_value())
		{
			...
			for (auto& attribute_entry : entity_type_attributes) {
				O_ENT_ATTR_TYPE o_attribute_type = entity_attribute_type_manager->create_entity_attribute_type(attribute_entry.first, attribute_entry.second, entity_type_attribute_features[attribute_entry.first]);
