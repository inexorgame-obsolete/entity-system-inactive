...
EntityInstancePtrOpt LogManager::register_logger(const std::string &logger_name)
{
    try
    {
        spdlog::register_logger(std::make_shared<spdlog::async_logger>(logger_name, sinks.begin(), sinks.end(), spdlog::thread_pool(), spdlog::async_overflow_policy::block));
...
void LogManager::unregister_logger(const std::string &logger_name)
{
    spdlog::drop(logger_name);
}

void LogManager::set_level(const std::string &logger_name, spdlog::level::level_enum level)
{
...
spdlog::level::level_enum LogManager::get_level(const std::string &logger_name)
{
    if (logger_instances.count(logger_name))
    {
...
EntityInstancePtrOpt LogManager::get_logger(const std::string &logger_name)
{
    if (logger_instances.count(logger_name))
    {
        return EntityInstancePtrOpt{logger_instances[logger_name]};
