...
class EntityInstanceManager : public InstanceManagerTemplate<EntityInstance>
{
    ...
    /// @return A shared pointer to the entity instance which was created.
    EntityInstancePtrOpt create_entity_instance(const EntityTypePtr &type, const std::function<bool(EntityInstancePtr)>& instance_initializer);
    ...
    /// @param ent_type A const reference of a shared pointer to an entity type of which an instance will be created.
    /// @return A shared pointer to the entity instance which was created.
    EntityInstancePtrOpt create_entity_instance(const xg::Guid &inst_GUID, const EntityTypePtr &type, const std::function<bool(EntityInstancePtr)>& instance_initializer);
    ...
    bool does_entity_instance_exist(xg::Guid instance_GUID);
    ...
    void register_on_created(const xg::Guid &type_GUID, const std::shared_ptr<EntityInstanceCreatedListener>& listener);

    /// @brief Registers a new listener.
    /// @param type_GUID The GUID of the type which was deleted.
    /// @param listener The listener.
    void register_on_deleted(const xg::Guid &type_GUID, const std::shared_ptr<EntityInstanceDeletedListener>& listener);
    ...
    void notify_entity_instance_created(const EntityInstancePtr& new_entity_instance);
