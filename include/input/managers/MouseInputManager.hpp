...
class MouseInputManager : public LifeCycleComponent, public std::enable_shared_from_this<MouseInputManager>
{
    ...
    MouseInputManager(GlobalMouseButtonFactoryPtr global_mouse_button_factory, LogManagerPtr log_manager);
    ...
    void mouse_position_changed(const EntityInstancePtr& window, double xpos, double ypos);

    /// @brief The state of the mouse button has been changed.
    void mouse_button_changed(const EntityInstancePtr& window, int button, int action, int mods);

    /// @brief The state of the mouse scroll has been changed.
    void mouse_scroll_changed(const EntityInstancePtr& window, double xoffset, double yoffset);

    /// @brief Registers a listener for mouse button change on any window.
    void register_on_mouse_button_changed(const std::shared_ptr<MouseButtonChangedListener>& mouse_button_changed_listener);

    /// @brief Registers a listener for mouse button press on any window.
    void register_on_mouse_button_pressed(const std::shared_ptr<MouseButtonPressedListener>& mouse_button_pressed_listener);

    /// @brief Registers a listener for mouse button release on any window.
    void register_on_mouse_button_released(const std::shared_ptr<MouseButtonReleasedListener>& mouse_button_released_listener);

    /// @brief Registers a listener for mouse button change on a specific window.
    void register_on_window_mouse_button_changed(const EntityInstancePtr& window, const std::shared_ptr<WindowMouseButtonChangedListener>& window_mouse_button_changed_listener);

    /// @brief Registers a listener for mouse button press on a specific window.
    void register_on_window_mouse_button_pressed(const EntityInstancePtr& window, const std::shared_ptr<WindowMouseButtonPressedListener>& window_mouse_button_pressed_listener);

    /// @brief Registers a listener for mouse button release on a specific window.
    void register_on_window_mouse_button_released(const EntityInstancePtr& window, const std::shared_ptr<WindowMouseButtonReleasedListener>& window_mouse_button_released_listener);

    /// @brief Registers a listener for a change of the mouse position on a specific window.
    void register_on_window_mouse_moved(const EntityInstancePtr& window, const std::shared_ptr<WindowMouseMovedListener>& window_mouse_moved_listener);

    /// @brief Registers a listener for mouse scroll on a specific window.
    void register_on_window_mouse_scrolled(const EntityInstancePtr& window, const std::shared_ptr<WindowMouseScrolledListener>& window_mouse_scrolled_listener);
